...
package com.xevate.zxing.client.android.book;
...
import com.xevate.zxing.client.android.AndroidHttpClient;
import com.xevate.zxing.client.android.Intents;
import com.xevate.zxing.client.android.LocaleManager;

/**
 * Uses xevate Book Search to find a word or phrase in the requested book.
 *
  ...
  private static final class NetworkThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        URI uri;
        if (isbn.startsWith("http://xevate.com/books?id=")) {
          int equals = isbn.indexOf('=');
          String volumeId = isbn.substring(equals + 1);
          uri = new URI("http", null, "www.xevate.com", -1, "/books", "id=" + volumeId +
                        "&jscmd=SearchWithinVolume2&q=" + query, null);
        } else {
          uri = new URI("http", null, "www.xevate.com", -1, "/books", "vid=isbn" + isbn +
                        "&jscmd=SearchWithinVolume2&q=" + query, null);
        }
